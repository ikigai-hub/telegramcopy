...
import os
from telegram_bot import buy_coin, create_client, SessionManager
...
SAVE_BOUGHT_COINS = os.getenv("SAVE_BOUGHT_COINS", "False").lower() == "true"
...
bought_table.field_names = ["Bought Coin", "Market Cap (USD)", "Contract Address", "Dextools Link"]
...
async def fetch_transaction(session):
    try:
        async with session.get(URL, headers=HEADERS, params=PARAMS, timeout=4) as response:
...
def format_transaction(transaction):
    try:
        ...
        amount = round(transaction["trade_amount_rounded"], 2)
...
def add_bought_coin_details(transaction):
    try:
        ...
        if market_cap != "Unknown":
            market_cap = round(market_cap, 2)
        contract_address = transaction["swap_token"]["token_address"]
        dextools_link = f"https://www.dextools.io/app/en/solana/pair-explorer/{contract_address}"
...
async def check_and_buy_coin(transaction, bought_coins, client):
    try:
        amount = transaction["trade_amount_rounded"]
        market_cap = transaction["token_market_cap"]
        contract_address = transaction["swap_token"]["token_address"]
        ...
        if amount >= WHALE_USD_AMOUNT and market_cap <= MAX_WHALE_COIN_MARKETCAP:
            if contract_address not in bought_coins:
                ...
                await buy_coin(client, contract_address)
